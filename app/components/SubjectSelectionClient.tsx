...
export default function SubjectSelectionClient({ classroom, initialSubjects }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await createSubject(newSubjectName, classroom.id);
    setNewSubjectName('');
  ...
  const handleDelete = async (id: string) => {
    await deleteSubject(id, classroom.id);
    setOpenMenuId(null);
  };
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto flex-1 overflow-y-auto w-full">
      <div className="flex items-center gap-2 text-xs font-mono text-lumina-text-muted mb-4 select-none">
        <Link href="/" className="hover:text-lumina-primary transition-colors cursor-pointer">
          Classes
        </Link>
        <span>&rsaquo;</span>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10">
        <div>
          ...
          <p className="text-sm text-lumina-text-muted max-w-2xl">
            Manage content, questions, and curriculum for your students.
          </p>
        </div>
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center z-50 p-4">
            ...
            >
              ...
              <p className="text-xs text-lumina-text-muted mb-6">
                Register a curriculum topic container.
              </p>
